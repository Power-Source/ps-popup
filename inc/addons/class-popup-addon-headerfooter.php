...
Addon Name: Test Header / Footer

Plugin URI:  http://gist.github.com/378450

Description: Testet die Existenz und Funktionalität von wp_head und wp_footer im aktiven Thema.
		...
		if ( false === $Init ) {
			...
			if ( ! strstr( $html, '<wp_head>exists</wp_head>' ) ) {
				...
				$Resp->msg[] = __(
					...
					'Kritisch: Aufruf von <code>wp_head();</code> fehlt! Es ' .

					'sollte direkt vor <code>&lt;/head&gt;</code> erscheinen', 'popover'
			...
			if ( ! strstr( $html, '<wp_footer>exists</wp_footer>' ) ) {
				...
				$Resp->msg[] = __(
					...
					'Kritisch: Aufruf von <code>wp_footer();</code> fehlt! Es ' .

					'sollte direkt vor <code>&lt;/body&gt;</code> erscheinen', 'popover'
			...
			if ( empty( $Resp->msg ) ) {
				...
				$Resp->msg[] = __(
					...
					'Okay: IDein aktuelles Theme verwendet <code>wp_head();</code>> und ' .

					'<code>wp_footer();</code> korrekt!', 'popover'
