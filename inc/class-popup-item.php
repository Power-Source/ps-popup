...
class IncPopupItem {
	...
	protected function validate_data() {
		...
		if ( ! $this->is_upfront && is_admin() && $this->id >= 0 ) {
			...
			if ( empty( $this->name ) ) {
				...
				$this->name = __( 'Neues PopUp', 'popover' );
		...
		if ( $show_message ) {
			...
			if ( ! empty( $res ) ) {
				...
				if ( $this->orig_status === $this->status ) {
					...
					$msg = __( 'PopUp "<strong>%1$s</strong>" gespeichert', 'popover' );
					...
					switch ( $status ) {
						...
						case 'publish':
							...
							$msg = __( 'PopUp "<strong>%1$s</strong>" aktiviert.', 'popover' );
						...
						case 'draft':
							...
							$msg = __( 'PopUp "<strong>%1$s</strong>" deaktiviert.', 'popover' );
						...
						case 'trash':
							...
							$msg = __( 'PopUp "<strong>%1$s</strong>" wurde in den Papierkorb verschoben.', 'popover' );
				...
				lib3()->ui->admin_message( __( 'PopUp konnte nicht gespeichert werden.', 'popover' ), 'err' );
		...
		if ( 'footer' !== $method
			...
			lib3()->ui->admin_message(
				...
				sprintf(
					...
					__( 'Du verwendest Gravity Forms in diesem PopUp. Wechsel am besten zur <a href="%s">Lademethode</a> "Seitenfooter", um sicherzustellen, dass das Formular wie erwartet funktioniert.', 'popover' ),
					...
					if ( preg_match( '#\[' . $code . '(\s.*?\]|\])#', $content, $match ) ) {
						...
						lib3()->ui->admin_message(
							...
							sprintf(
								...
								__( 'Für den Shortcode <code>%s</code> ist eine andere <a href="%s">Lademethode</a> erforderlich. <br /> Versuche "Seitenfooter", manchmal jedoch auch die Methode "Benutzerdefiniertes AJAX" funktioniert (bitte teste das Ergebnis)', 'popover' ),
	...
	protected function prepare_template_vars() {
		...
		if ( $this->can_hide ) {
			...
			$hide_forever_tag = sprintf(
				...
				__( 'Nicht mehr anzeigen.', 'popover' )
	...
	static public function condition_label( $key = null ) {
		...
		switch ( $key ) {
			...
			case 'login':        return __( 'Besucher ist angemeldet', 'popover' );

			case 'no_login':     return __( 'Besucher ist nicht angemeldet', 'popover' );

			case 'url':          return __( 'Auf einer bestimmten URL', 'popover' );

			case 'no_url':       return __( 'Nicht auf einer bestimmten URL', 'popover' );

			case 'country':      return __( 'In einem bestimmten Land', 'popover' );

			case 'no_country':   return __( 'Nicht in einem bestimmten Land', 'popover' );

			case 'prosite':      return __( 'Seite ist keine Bloghosting Pro-Seite', 'popover' );

			case 'searchengine': return __( 'Besuche über eine Suchmaschine', 'popover' );

			case 'no_comment':   return __( 'Besucher hat noch nie kommentiert', 'popover' );

			case 'no_internal':  return __( 'Besuche über keinen internen Link', 'popover' );

			case 'referrer':     return __( 'Besuch über einen bestimmten Referer', 'popover' );

			case 'count':        return __( 'Popover wird weniger als x-mal angezeigt', 'popover' );
	...
	static public function status_label( $key ) {
		...
		switch ( $key ) {
			...
			case 'active':    return __( 'Aktiv', 'popover' );

			case 'inactive':  return __( 'Inaktiv', 'popover' );

			case 'trash':     return __( 'Papierkorb', 'popover' );
