...
Name:        Besucher Standort

Plugin URI:  https://n3rds.work/piestingtal-source-project/ps-popup/

Description: Bedingungen basierend auf dem Standort des Besuchers.
...
Rules:       In einem bestimmten Land, Nicht in einem bestimmten Land
...
class IncPopupRule_Geo extends IncPopupRule {
	...
	protected function init() {
		...
		$this->add_rule(
			...
			__( 'In einem bestimmten Land', 'popover' ),

			__( 'Zeigt das PopUp an, wenn sich der Benutzer in einem bestimmten Land befindet.', 'popover' ),
		...
		$this->add_rule(
			...
			__( 'Nicht in einem bestimmten Land', 'popover' ),

			__( 'Zeigt das PopUp an, wenn sich der Benutzer nicht in einem bestimmten Land befindet.', 'popover' ),
	...
	protected function form_country( $data ) {
		...
		<select name="po_rule_data[country][]"
			...
			placeholder="<?php _e( 'Klicke hier, um ein Land auszuwählen', 'popover' ); ?>">
	...
	protected function form_no_country( $data ) {
		...
		<label for="po-rule-data-no-country">
			...
			<?php _e( 'Ausgeschlossene Länder:', 'popover' ); ?>
	...
	static protected function get_service( $type = null ) {
		...
		if ( '' == $type ) {
			...
			$service = (object) array(
				...
				'label' => __( 'Lokale IP Lookup Tabelle', 'popover' ),
