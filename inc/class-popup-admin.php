...
class IncPopup extends IncPopupBase {
		...
		if ( IncPopupItem::POST_TYPE === $hook->post_type ) {
			...
			if ( 'trash' != $_REQUEST['post_status'] ) {
				lib3()->ui->data(
					'po_bulk',
					array(
						'activate' => __( 'Aktivieren', 'popover' ),
						'deactivate' => __( 'Deaktivieren', 'popover' ),
						'toggle' => __( 'Aktivierung umschalten', 'popover' ),
					)
				);
			}
	...
	static public function admin_menus() {
		...
		if ( is_network_admin() ) {
			...
			$submenu[ IncPopupItem::POST_TYPE . '-list' ][0][0] = _x( 'Globale PopUps', 'Beitragstyp Allgemeiner Name', 'popover' );
	...
	static public function handle_settings_update() {
		...
		if ( 'updatesettings' == $_POST['action'] ) {
			...
			if ( $old_settings['geo_lookup'] != $settings['geo_lookup'] ) {
				...
				lib3()->ui->admin_message( __( 'Country Lookup geändert: Der Lookup-Cache wurde gelöscht.', 'popover' ) );
			...
			lib3()->ui->admin_message( __( 'Deine Einstellungen wurden aktualisiert.', 'popover' ) );
	...
	static public function post_columns( $post_columns ) {
		...
		$new_columns['po_cond'] = __( 'Bedingungen', 'popover' );



		if ( 'trash' !== $_REQUEST['post_status'] ) {

			$new_columns['po_pos'] = __( 'Reihenfolge', 'popover' );

			$new_columns['po_state'] = __( 'Aktiv', 'popover' );
		...
		switch ( $column ) {
			...
			case 'po_name':
				...
				if ( $can_edit ) {
					...
					$actions['edit'] = array(
						...
						'title' => __( 'Bearbeite dieses PopUp', 'popover' ),

						'attr' => '',

						'label' => __( 'Bearbeiten', 'popover' ),
				...
				if ( $can_edit && 'active' === $popup->status ) {
					...
					$actions['deactivate'] = array(
						...
						'title' => __( 'Deaktiviere dieses PopUp', 'popover' ),

						'attr' => '',

						'label' => __( 'Deaktivieren', 'popover' ),
				...
				if ( $can_edit && 'inactive' === $popup->status ) {
					...
					$actions['activate'] = array(
						...
						'title' => __( 'Aktiviere dieses PopUp', 'popover' ),

						'attr' => '',

						'label' => __( 'Aktivieren', 'popover' ),
				...
				$actions['popup_preview'] = array(
					...
					'title' => __( 'Vorschau dieses PopUps', 'popover' ),

					'attr' => 'class="po-preview" data-id="' . $post_id . '"',

					'label' => __( 'Vorschau', 'popover' ),
				...
				if ( current_user_can( 'delete_post', $post_id ) ) {
					...
					if ( 'trash' === $popup->status ) {
						...
						$actions['untrash'] = array(
							...
							'title' => __( 'Stelle dieses PopUp aus dem Papierkorb wieder her', 'popover' ),

							'attr' => '',

							'label' => __( 'Wiederherstellen', 'popover' ),
						...
						$actions['trash'] = array(
							...
							'title' => __( 'Verschiebe dieses PopUp in den Papierkorb', 'popover' ),

							'attr' => 'class="submitdelete"',

							'label' => __( 'Papierkorb', 'popover' ),
					...
					if ( 'trash' === $popup->status || ! EMPTY_TRASH_DAYS ) {
						...
						$actions['delete'] = array(
							...
							'title' => __( 'Lösche dieses PopUp dauerhaft', 'popover' ),

							'attr' => 'class="submitdelete"',

							'label' => __( 'Dauerhaft löschen', 'popover' ),
				...
				if ( $can_edit ) : ?>
					...
					<a href="<?php echo esc_url( get_edit_post_link( $post_id ) ); ?>"
						...
						title="<?php _e( 'Bearbeite dieses PopUp', 'popover' ); ?>">
			...
			case 'po_cond':
				...
				<?php if ( ! $rule_count ) : ?>
					...
					<span class="rule-always"><?php _e( 'PopUp immer anzeigen', 'popover' ); ?></span>
	...
	static public function post_views( $views ) {
		...
		$stati = array(
			...
			'all' => __( 'Alle <span class="count">(%1$s)</span>', 'popover' ),

			'publish' => __( 'Aktive <span class="count">(%1$s)</span>', 'popover' ),

			'draft' => __( 'Inaktive <span class="count">(%1$s)</span>', 'popover' ),

			'trash' => __( 'Papierkorb <span class="count">(%1$s)</span>', 'popover' ),
	...
	static public function post_update_messages( $messages, $counts ) {
		...
		$messages[ IncPopupItem::POST_TYPE ] = array(
			...
			'updated'   => _n( 'Ein PopUp aktualisiert.', '%s PopUps aktualisiert.', $counts['updated'] ),

			'locked'    => _n( 'Ein PopUp nicht aktualisiert, jemand bearbeitet es.', '%s PopUps nicht aktualisiert, jemand bearbeitet sie.', $counts['locked'] ),

			'deleted'   => _n( 'Ein PopUp dauerhaft gelöscht.', '%s PopUps dauerhaft gelöscht.', $counts['deleted'] ),

			'trashed'   => _n( 'Ein PopUp wurde in den Papierkorb verschoben.', '%s PopUps sind in den Papierkorb gewandert.', $counts['trashed'] ),

			'untrashed' => _n( 'Ein PopUp aus dem Papierkorb wiederhergestellt.', '%s PopUps aus dem Papierkorb wiederhergestellt.', $counts['untrashed'] ),
			...
			if ( 'list' === $_REQUEST['mode'] ) {
				...
				switch ( $action ) {
					...
					case 'activate':
						...
						$msg = __( 'Ein PopUp aktiviert', 'popover' ) :

						$msg = __( '%1$s PopUps aktiviert', 'popover' );
					...
					case 'deactivate':
						...
						$msg = __( 'Ein PopUp ist deaktiviert', 'popover' ) :

						$msg = __( '%1$s PopUps deaktiviert', 'popover' );
					...
					case 'toggle':
						...
						$msg = __( 'Ein PopUp umgeschaltet', 'popover' ) :

						$msg = __( '%1$s PopUps umgeschaltet', 'popover' );
	...
	static public function form_metabox( $post ) {
		...
		add_meta_box(
			...
			__( 'Anzeigebedingungen (optional)', 'popover' ),
		...
		add_meta_box(
			...
			__( 'Benutzerdefiniertes CSS (optional)', 'popover' ),
	...
	static public function form_title( $post ) {
		...
		<div id="titlediv">
			...
			<div id="titlewrap">
				...
				<label for="po_name"><?php _e( 'PopUp Name (wird im PopUp nicht angezeigt)', 'popover' ); ?></label>

				<input type="text" id="po_name" name="po_name" required

					value="<?php echo esc_attr( $popup->name ); ?>"

					placeholder="<?php _e( 'Gib dem PopUp einen Namen', 'popover' ); ?>" />
